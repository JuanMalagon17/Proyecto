...
Por qué está separado de FacturaController:
  ...
  afectar en nada el ciclo de vida de una factura, y viceversa.
...
class CarteraController:
    ...
    # Resumen ejecutivo
    # ------------------------------------------------------------------ #
    def calcular_resumen_cartera(self) -> Dict:
        """
    ...
    def top_clientes_por_ventas(self, n: int = 5) -> List[Tuple[str, float]]:
        ...
        for f in self._factura_dao.listar_todos():
            if f.estado != EstadoFactura.ANULADA:
                acumulado[f.id_cliente] = acumulado.get(f.id_cliente, 0) + f.total
    ...
    # ------------------------------------------------------------------ #
    def rentabilidad_por_producto(self) -> List[Dict]:
        """
        ...
        for f in self._factura_dao.listar_todos():
            ...
            for linea in f.lineas:
                ...
                acumulado[pid]["total_vendido"] += linea.subtotal
                acumulado[pid]["costo_total"]   += linea.costo_total
                acumulado[pid]["unidades"]      += linea.cantidad

        resultado = []
        for d in acumulado.values():
            utilidad = d["total_vendido"] - d["costo_total"]
            margen   = (utilidad / d["total_vendido"] * 100) if d["total_vendido"] > 0 else 0.0
            resultado.append({
                "id_producto"      : d["id_producto"],
                "nombre"           : d["nombre"],
                "unidades_vendidas": d["unidades"],
                "total_vendido"    : round(d["total_vendido"], 2),
                "utilidad_bruta"   : round(utilidad, 2),
                "margen_pct"       : round(margen, 2),
            })

        return sorted(resultado, key=lambda x: x["utilidad_bruta"], reverse=True)

    # ------------------------------------------------------------------ #
    # Análisis de morosidad
    # ------------------------------------------------------------------ #
    def calcular_indice_morosidad(self) -> Dict:
        """
